...
class BasePredictor(object):
    def __init__(
        ...
        **kwargs
        ...
        self.dist_maps = DistMaps3D(
            norm_radius=norm_radius, spatial_scale=spatial_scale, cpu_mode=False, use_disks=True
        )
    ...
    def set_input_image(self, image):  # (1, 12, 512, 512)
        ...
        self.original_image = paddle.to_tensor(image).astype("float32") / 255  # 1, 512, 512, 12]

        for transform in self.transforms:
            transform.reset()

        if len(self.original_image.shape) == 4:
            self.original_image = self.original_image.unsqueeze(0)  # (1, 1, 12, 512, 512)
    ...
    def get_prediction_noclicker(self, click, prev_mask=None):
        ...
        input_image = paddle.concat([input_image, prev_mask], axis=1)  # [1, 2, 512, 512, 12]
    ...
    def get_prediction(self, clicker, prev_mask=None):
        ...
        prediction = F.interpolate(
            pred_logits, mode="trilinear", align_corners=True, size=image_nd.shape[2:], data_format="NCDHW"
        )
    ...
    def get_coord_features(self, image, prev_mask, points):
        ...
        if prev_mask is not None:
            coord_features = paddle.concat((prev_mask, coord_features), axis=1)  # [1, 3, 512, 512, 12]

        return coord_features

    def _get_prediction(self, image_nd, clicks_lists, is_image_changed):  # what is the click? click on the right place?
