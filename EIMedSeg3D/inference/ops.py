...
class BatchImageNormalize3D:  # 标准化 均值为0，方差为1
    def __init__(self, mean, std):
        self.mean = paddle.to_tensor(np.array(mean)[np.newaxis, :, np.newaxis, np.newaxis, np.newaxis]).astype(
            "float32"
        )
        self.std = paddle.to_tensor(np.array(std)[np.newaxis, :, np.newaxis, np.newaxis, np.newaxis]).astype("float32")
...
class ScaleLayer(nn.Layer):
    def __init__(self, init_value=1.0, lr_mult=1):
        ...
        self.scale = self.create_parameter(
            shape=[1], dtype="float32", default_initializer=nn.initializer.Constant(init_value / lr_mult)
        )
...
class DistMaps3D(nn.Layer):
    def __init__(self, norm_radius, spatial_scale=1.0, cpu_mode=False, use_disks=False):  # (1, 1.0, False, True)
    ...
    def get_coord_features(self, points, batchsize, rows, cols, layers):  # [B, num_points*2, 4]
        if self.cpu_mode:
            coords = []
            for i in range(batchsize):
                norm_delimeter = 1.0 if self.use_disks else self.spatial_scale * self.norm_radius
                coords.append(self._get_dist_maps(points[i].numpy().astype("float32"), rows, cols, norm_delimeter))
            coords = paddle.to_tensor(np.stack(coords, axis=0)).astype("float32")
        else:
            num_points = points.shape[1] // 2  # [1, 2, 4]
            points = points.reshape([-1, paddle.shape(points)[2]])  # [B*num_points*2, 4]
            points, points_order = paddle.split(points, [3, 1], axis=1)  # [2, 3]
            ...
            layer_array = paddle.arange(start=0, end=layers, step=1, dtype="float32")

            coord_rows, coord_cols, coor_layers = paddle.meshgrid(
                row_array, col_array, layer_array  # [512, 512, 12]
            )  # len is 3 [rows, cols, layers]
            coords = paddle.unsqueeze(
                paddle.stack([coord_rows, coord_cols, coor_layers], axis=0), axis=0
            ).tile(  # [B*num_points*2, 3, rows, cols, layers]
                [paddle.shape(points)[0], 1, 1, 1, 1]
            )  # [B*num_points*2 | 768, 3, 512, 512, 12] # repeat

            add_xy = (points * self.spatial_scale).reshape([points.shape[0], points.shape[1], 1, 1, 1])
        ...
        if self.use_disks:
            coords = (coords <= (self.norm_radius * self.spatial_scale) ** 2).astype("float32")  # 只取较小的数值对应的特征
        else:
